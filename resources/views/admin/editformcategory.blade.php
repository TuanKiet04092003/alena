      ...
      <form action="/admin/editcategory" method="post">
        @csrf
              ...
              <div class="row my-2">
                ...
                <div class="col-9">
                  <input type="text" name="stt" class="form-control" placeholder="Nhập thứ tự xuất hiện" value="{{ $category->stt }}">
              ...
              <div class="row my-2">
                ...
                <div class="col-9">
                  <select name="sethome" id="" class="form-select" value="{{ $category->sethome }}">
                    <option {{ $category->sethome==0?'selected':'' }} value="0">Không xuất hiện</option>
                    <option {{ $category->sethome==1?'selected':'' }} value="1">Xuất hiện</option>
                  </select>
                </div>
              </div>
              <div class="row my-2">
                ...
                <div class="col-9">
                  <select name="id_parent" id="" class="form-select">
                    <option {{ $category->id_parent==0?'selected':'' }} value="0">Không thuộc danh mục nào</option>
                    @foreach ($categories as $item)
                      <option {{ $category->id_parent==$item->id?'selected':'' }} value="{{ $item->id }}">{{ $item->name }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
