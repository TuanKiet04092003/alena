        ...
        <div class="row">
            <div class="col-xl-6 col-lg-7 col-md-10 col-sm-12 mx-auto">
                <div class="login-form col-6 py-5 rounded-4">
                    ...
                    <h2 class="text-secondary text-center fw-semibold">Xác nhận</h2>
                    <p class=" text-center mb-4">Nếu bạn chưa có tài khoản, đăng ký <a href="/register"
                            class="text-secondary">tại đây</a></p>
                    <form action="/verify" method="post">
                        @csrf
                        <div class="w-75 mx-auto">
                            <input type="text" class="form-control w-100 my-3 p-2" placeholder="Mã xác nhận" name="verifycode" value="{{ old('verifycode') }}">
                            @if ($errors->has('verifycode'))
                                <div class="text-danger mb-2">{{ $errors->first('verifycode') }}</div>
                            @endif
                            @if ($errors->has('created_at'))
                                <div class="text-danger mb-2">{{ $errors->first('created_at') }}</div>
                            @endif
